create table if not exists public.stock_refresh_leases (
  ...
  primary key (kind, market, symbol, view_mode),
  constraint stock_refresh_leases_view_mode_check check (view_mode in ('', 'detail', 'compare'))
);

create index if not exists stock_refresh_leases_until_idx
on public.stock_refresh_leases (lease_until);

drop trigger if exists set_stock_refresh_leases_updated_at
on public.stock_refresh_leases;

create trigger set_stock_refresh_leases_updated_at
before update on public.stock_refresh_leases
for each row
execute function public.set_updated_at();

alter table public.stock_refresh_leases enable row level security;

revoke all on table public.stock_refresh_leases from public;
grant select, insert, update, delete on table public.stock_refresh_leases to service_role;
...
begin
  ...
  if normalized_view is not null and normalized_view not in ('detail', 'compare') then
